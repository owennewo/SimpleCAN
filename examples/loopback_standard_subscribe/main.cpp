...
void receiveCallback() // <- This function's address is passed below as argument to can.subscribe)
{
    // this is an interrupt callback, we must return quickly, we should read the frame to remove from RxFifo
    if (can.readFrame(&rxFrame) == CAN_OK)
    {
        // might want to unset CAN_DEBUG build flag to reduce latency
        digitalWrite(LED_BUILTIN, digitalRead(LED_BUILTIN)); //<- toggle LED
        Serial.println("✅");                                // <- might want to remove this line to reduce latency.
    }
}
...
void setup()
{
    ...
    can.filter(FILTER_ACCEPT_ALL_STANDARD); // <- accepting all *STANDARD* frames, without this readFrame won't work
    can.subscribe(&receiveCallback);        // subscribe to receive a callback (in interrupt routine)
    can.start();                            // <- start CAN peripheral, if CAN_SHDN is defined, it will also enable CAN transceiver

    // prepare frame
    txFrame.identifier = 0x321; // <- 11-bit identifier (extended).
